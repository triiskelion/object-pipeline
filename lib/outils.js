...
let outils = {
  ...
  Objects: {
    ...
    getValueByPath: function (source, path) {
      ...
      let node = source
      for ( let key of path ) {
        if ( !hasOwnProperty.call(node, key) ) { return undefined }
        node = node[key]
      }
      return node
    },

    createObjectByPath: function (value, path) {
      return createObjectByPath(value, path)
    }
...
function createObjectByPath (value, path) {

  let key = path.shift()

  let child = path.length == 0 ? value : createObjectByPath(value, path)
  let ret
  if ( outils.Type.isNumber(key) ) {
    ret = []
